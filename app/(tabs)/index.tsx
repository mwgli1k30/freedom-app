...
export default function FreedomApp() {
  ...
  const getCoachReply = (msg, history) => {
    const m = msg.toLowerCase();
    const histLen = history.filter(h => h.role === "assistant").length;

    // Détection mots-clés
    const isEnvie = /envie|impuls|tentation|attir|excit|porno|rechut|craqu/.test(m);
    const isMal = /mal|triste|déprim|nul|honte|verg|dégoût|inutile|raté|échec|honte/.test(m);
    const isFatigue = /fatigué|épuisé|dormi|sommeil|nuit|réveil|soir/.test(m);
    const isSeul = /seul|isol|personne|ami|famille|abandon/.test(m);
    const isStress = /stress|anxieux|anxiété|angoiss|peur|inquiet|travail|exam|pression/.test(m);
    const isMotivé = /motivé|bien|fort|fier|réussi|tenu|jours|streak|progres/.test(m);
    const isQuestion = /comment|pourquoi|quoi|aide|conseil|que faire|aidez/.test(m);
    const isRechuté = /rechuté|rechute|craqué|recommenc|encore|échoué|raté/.test(m);
    const isBored = /ennui|rien|vide|long|temps|occu/.test(m);

    const pools = {
      envie: [
        `Cette envie que tu ressens, c'est normal — ton cerveau réclame sa dose de dopamine. Mais elle va passer dans 90 secondes si tu passes à l'action. Va faire 20 pompes maintenant, sans négocier.`,
        `L'envie est là, c'est un signal — pas un ordre. Ton cerveau teste ta résistance. Ouvre le mode SOS et choisis un exercice immédiatement. Chaque fois que tu résistes, tu renforces le nouveau circuit.`,
        `Pose le téléphone, lève-toi et va dans une autre pièce. Physiquement. L'environnement change le cerveau. Une fois debout, appelle quelqu'un ou fais couler l'eau froide.`,
        `Ce que tu ressens là, c'est de la neurochimie, pas toi. Le circuit de récompense s'active. Counter-attaque avec quelque chose de physique et intense. Sport, douche froide, sortir dehors.`,
      ],
      mal: [
        `C'est courageux d'être là et de le dire. Qu'est-ce qui se passe exactement ? Quelque chose de précis s'est passé aujourd'hui, ou c'est un poids général ?`,
        `Ce sentiment est valide. Beaucoup de gens dans ce programme passent par là — surtout en début de parcours. Le cerveau se désintoxique et ça crée du vide. Ça passe, mais dis-moi ce qui te pèse le plus là.`,
        `Mal comment ? Honte, tristesse, vide ? Je ne juge pas, j'ai juste besoin de comprendre pour t'aider concrètement. Tu n'es pas seul dans ça.`,
      ],
      fatigue: [
        `La fatigue amplifie tout — les envies, les doutes, les émotions négatives. Ton cerveau en sevrage consomme plus d'énergie que d'habitude. Ce soir, couche-toi 30 minutes plus tôt que prévu. C'est un acte de discipline.`,
        `Fatigué, c'est exactement le moment où le cerveau cherche un raccourci facile. Mange quelque chose de nourrissant, bois de l'eau, et mets-toi au lit. Demain tu verras les choses différemment.`,
        `Le sommeil est une arme contre l'addiction. Un cerveau reposé résiste 3x mieux aux impulsions. Ce soir, téléphone en mode avion, lumières tamisées, pas d'écran 30 min avant de dormir.`,
      ],
      seul: [
        `La solitude est le terrain le plus fertile pour l'addiction. Ce n'est pas un défaut — c'est humain. Mais là, maintenant, envoie un message à quelqu'un. N'importe qui. Même banal.`,
        `Tu n'es pas seul dans ce combat — des milliers de personnes utilisent Freedom en ce moment. Et je suis là. Mais au-delà de moi, qui dans ta vie pourrait recevoir un appel de toi ce soir ?`,
        `L'isolement nourrit les mauvaises habitudes. Ton défi du jour : un contact humain réel. Message, appel, rencontre. Pas les réseaux. Une vraie conversation.`,
      ],
      stress: [
        `Le stress active le même circuit que l'addiction — c'est pour ça que ton cerveau cherche un exutoire facile. Mais ça ne résout pas le stress, ça l'amplifie. Qu'est-ce qui te stresse là précisément ?`,
        `Stress et addiction sont liés neurochimiquement. La solution n'est pas la suppression mais la redirection. 5 minutes de respiration 4-7-8 maintenant, puis on parle de ce qui te pèse.`,
        `Ce que tu ressens est réel. Le stress monte et le cerveau cherche du soulagement rapide. Mais tu sais que ce soulagement est temporaire et coûteux. Dis-moi ce qui se passe.`,
      ],
      motivé: [
        `C'est exactement ça. Ce sentiment de clarté et de force que tu ressens, c'est ton cerveau qui se recâble. Garde ce moment en mémoire pour les moments difficiles.`,
        `Bien. Capitalise sur ça. Ajoute une entrée dans ton journal maintenant — décris exactement ce que tu ressens. Tu en auras besoin les jours difficiles.`,
        `Cette progression est réelle et méritée. La prochaine étape : augmenter la difficulté. Quel défi tu n'as pas encore fait ? Fais-le aujourd'hui pendant que tu es dans cet état.`,
      ],
      rechuté: [
        `Ça arrive. La rechute fait partie du processus pour beaucoup de gens — ce n'est pas une raison d'abandonner, c'est une donnée. Qu'est-ce qui s'est passé juste avant ? Qu'est-ce qui a déclenché ça ?`,
        `Tu n'es pas un échec parce que tu as rechuté. Tu es quelqu'un qui apprend. La question n'est pas "pourquoi j'ai craqué" mais "qu'est-ce que je vais changer cette fois". Qu'est-ce qui t'a manqué ?`,
        `La rechute ne remet pas à zéro ce que ton cerveau a appris. Les neurones que tu as renforcés existent encore. Reprends le compteur, note le déclencheur dans ton journal, et repars. Maintenant.`,
      ],
      ennui: [
        `L'ennui est l'un des déclencheurs les plus sous-estimés. Ton cerveau cherche de la stimulation — donne-lui en. Sors dehors maintenant, même 10 minutes. Le changement d'environnement brise le schéma.`,
        `Le vide que tu ressens est réel. C'est ton cerveau qui s'est habitué à une stimulation artificielle intense. Il apprend à nouveau à apprécier les choses simples — ça prend du temps. Pour l'instant : bouge.`,
        `Ennui = danger. C'est le moment de sortir la liste des défis et d'en choisir un. Lequel tu n'as pas encore fait ?`,
      ],
      question: [
        `Bonne question. La première chose à comprendre : tu ne combats pas une habitude, tu recâbles un circuit neuronal. Ça demande de la répétition et de la patience. Qu'est-ce qui te bloque concrètement ?`,
        `Dis-moi quelle est ta situation exacte et je te donne un plan d'action précis. Depuis combien de temps tu essaies ? Quels sont tes déclencheurs principaux ?`,
        `Je peux t'aider à construire une stratégie personnalisée. Pour ça j'ai besoin de savoir : c'est quoi le moment le plus difficile de ta journée ?`,
      ],
      default: [
        `Je t'écoute. Continue — qu'est-ce qui se passe vraiment ?`,
        `Dis-moi en plus. Depuis quand tu ressens ça ?`,
        `Je comprends. Et concrètement, là maintenant, qu'est-ce dont tu as besoin ?`,
        `C'est important ce que tu dis. Comment tu te sens physiquement en ce moment ?`,
        `Qu'est-ce qui t'a amené à ouvrir l'app là maintenant ?`,
        `Tu fais bien d'en parler. Qu'est-ce qui est le plus pesant pour toi en ce moment ?`,
      ],
    };

    let pool = pools.default;
    if (isRechuté) pool = pools.rechuté;
    else if (isEnvie) pool = pools.envie;
    else if (isMal) pool = pools.mal;
    else if (isFatigue) pool = pools.fatigue;
    else if (isSeul) pool = pools.seul;
    else if (isStress) pool = pools.stress;
    else if (isMotivé) pool = pools.motivé;
    else if (isBored) pool = pools.ennui;
    else if (isQuestion) pool = pools.question;

    // Évite de répéter la même réponse
    const lastAssistant = history.filter(h => h.role === "assistant").slice(-3).map(h => h.text);
    const available = pool.filter(r => !lastAssistant.includes(r));
  ...
  const sendChat = async () => {
    ...
    const reply = getCoachReply(userMsg, newHistory);
  ...
  return (
    <div style={S.app}>
            ...
            {!startDate ? (
              <div>
                ...
                <div style={{ textAlign: "center", marginBottom: 24 }}>
                  <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 30, color: "#e2e2e2", letterSpacing: -0.5, marginBottom: 8 }}>Prêt à te libérer ?</div>
                  <div style={{ fontSize: 13, color: "#444", lineHeight: 1.7, maxWidth: 280, margin: "0 auto" }}>Lance ton programme. Chaque heure compte.</div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
                  {[{ v: "90", l: "jours" }, { v: "13", l: "défis" }, { v: "24h", l: "coach" }].map(({ v, l }) => (
                    <div key={l} style={{ ...S.card, marginBottom: 0, textAlign: "center", padding: "14px 8px" }}>
                      <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 26, color: "#e2e2e2", marginBottom: 4 }}>{v}</div>
                      <div style={{ fontSize: 9, color: "#333", letterSpacing: 1 }}>{l.toUpperCase()}</div>
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
                  {[
                    { icon: "⚡", title: "Défis quotidiens", desc: "13 défis concrets sur 90 jours.", color: "#fbbf24" },
                    { icon: "💬", title: "Coach IA 24h/24", desc: "Disponible à tout moment, sans jugement.", color: "#60a5fa" },
                    { icon: "🆘", title: "Mode SOS", desc: "Pour les moments les plus difficiles.", color: "#f87171" },
                  ].map((f, i) => (
                    <div key={i} style={{ ...S.card, marginBottom: 0, display: "flex", alignItems: "center", gap: 14 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 12, background: "#0d0d0d", border: "1px solid #1c1c1c", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{f.icon}</div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 2 }}>{f.title}</div>
                        <div style={{ fontSize: 11, color: "#444" }}>{f.desc}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "#0a0a0a", borderTop: "1px solid #181818", display: "flex", padding: "8px 0 18px" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setActiveChallenge(null); }} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, fontFamily: "inherit" }}>
            <div style={{ width: 36, height: 36, borderRadius: 12, background: tab === t.id ? "#1a1a1a" : "transparent", border: tab === t.id ? "1px solid #2a2a2a" : "1px solid transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, transition: "all 0.2s" }}>
              {t.icon}
            </div>
            <span style={{ fontSize: 8, fontWeight: 600, color: tab === t.id ? "#e2e2e2" : "#2a2a2a", letterSpacing: 0.5 }}>{t.label.toUpperCase()}</span>
          </button>
        ))}
      </div>
